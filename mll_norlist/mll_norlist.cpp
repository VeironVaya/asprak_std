...
void insertFirstRelasi(adrMK mk, adrMhs mhs, adrRelasi r) {
    r->toMhs = mhs;
    if (mk->firstRelasi == nullptr) {
        mk->firstRelasi = r;
    } else {
        r->nextRelasi = mk->firstRelasi;
        mk->firstRelasi = r;
    }
}
...
void hapusSemuaRelasi(adrMK mk) {
    adrRelasi currentR = mk->firstRelasi;
...
void deleteMhsDariMK(listMK &LMK, string namaMK, string nim) {
    ...
    adrRelasi current = mk->firstRelasi;
    ...
    if (found) {
        if (prev == nullptr) {
            mk->firstRelasi = current->nextRelasi;
        } else {
